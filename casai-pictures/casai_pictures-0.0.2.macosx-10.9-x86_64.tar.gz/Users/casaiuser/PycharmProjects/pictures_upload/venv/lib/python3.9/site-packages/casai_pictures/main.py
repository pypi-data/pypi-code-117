...
async def download_from_folder(
    ...
    **kwargs,
    ...
    global listings_download_stack, stored_files
...
async def upload_from_folder(
    ...
    **kwargs,
    ...
    """
    global success_count, not_found, errors, listings_upload_stack, stored_files

    print("\nAttempting upload for listing '{}'".format(nickname))
    file_extension = new_format
    timeout_counter = 0
    while len(listings_download_stack[nickname]) == 0:
        if stored_files == 0 and timeout_counter > 60:
            print("Found no pictures to upload!")
            break
        await asyncio.sleep(0.1)
        timeout_counter += 0.1

    while (
        listings_download_stack[nickname] != listings_upload_stack[nickname]
    ):
        filename = None
...
async def compress_and_upload(
    ...
    **kwargs,
    ...
    original_compressed, new_format_path = await compress_and_format(filename, file_extension)
